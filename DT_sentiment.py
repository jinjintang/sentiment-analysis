...
if __name__=='__main__':
    ...
    #dt_sentiment.fit(bag_of_words[:len(train)],np.array(train['sentiment']))

    dt_sentiment.fit(np.array(train['tfidf'].to_list()),np.array(train['sentiment']).reshape(-1,1))
    #y_pred=dt_sentiment.predict(bag_of_words[:len(train)])
    y_label=np.array(train['sentiment']).reshape(-1,1)
    y_pred=dt_sentiment.predict(np.array(train['tfidf'].to_list()))
    '''
    ...
    y_pred=dt_sentiment.predict(np.array(test['tfidf'].to_list()))
    y_pred=pd.DataFrame(y_pred)
    y_pred.index=test['number']
    for i in range(len(y_pred)):
        print(test.iloc[i]['number'],y_pred.iloc[i][0])
