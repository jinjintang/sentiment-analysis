...
if __name__=='__main__':
    ...
    bnb_topic= BernoulliNB(fit_prior=True) 
    
    train=data_all[:len(train)]
    test=data_all[len(train):]

    bnb_topic.fit(bag_of_words[:len(train)],np.array(train['topic']))

    #bnb_topic.fit(np.array(train['tfidf'].to_list()),np.array(train['topic']).reshape(-1,1))
    y_pred=bnb_topic.predict(bag_of_words[:len(train)])
    y_label=np.array(train['topic']).reshape(-1,1)
    #y_pred=bnb_topic.predict(np.array(train['tfidf'].to_list()))
    '''
    ...
    #y_pred=bnb_topic.predict(np.array(test['tfidf'].to_list()))
