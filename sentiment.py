...
if __name__=='__main__':
    ...
    word_training_dict=word_freq.most_common(200)


    word_training=[]
    for w in word_training_dict:
        word_training.append(w[0])
    ...
    dt_sentiment=DecisionTreeClassifier(criterion='entropy',min_samples_split=20,random_state=0)
    
    train=data_all[:len(train)]
    test=data_all[len(train):]


    dt_sentiment.fit(np.array(train['tfidf'].to_list()),np.array(train['sentiment']).reshape(-1,1))
    y_label=np.array(train['sentiment']).reshape(-1,1)
    y_pred1=dt_sentiment.predict_proba(np.array(train['tfidf'].to_list()))

    
    y_pred1=dt_sentiment.predict_proba(np.array(test['tfidf'].to_list()))
    word_training=words
    ...
    y_pred2=bnb_sentiment.predict_proba(bag_of_words[:len(train)])
    y_label=np.array(train['sentiment']).reshape(-1,1)
   
    y_pred2=bnb_sentiment.predict_proba(bag_of_words[len(train):])

    mnb_sentiment= MultinomialNB(fit_prior=True) 
    train=data_all[:len(train)]
    test=data_all[len(train):]

    mnb_sentiment.fit(bag_of_words[:len(train)],np.array(train['sentiment']))
    
    #mnb_sentiment.fit(np.array(train['tfidf'].to_list()),np.array(train['sentiment']).reshape(-1,1))
    y_pred3=mnb_sentiment.predict_proba(bag_of_words[:len(train)])
    y_label=np.array(train['sentiment']).reshape(-1,1)
    #y_pred=mnb_sentiment.predict_proba(np.array(train['tfidf'].to_list()))
  
   

    y_pred3=mnb_sentiment.predict_proba(bag_of_words[len(train):])
    y_pred=y_pred1+y_pred2+y_pred3
    str_ans=['negative','neutral','positive']
    y_pred=np.argmax(y_pred,axis=1)
    y_pred=pd.Series(y_pred).apply(lambda x:str_ans[x])
    '''
