...
if __name__=='__main__':
    ...
    y_pred=mnb_sentiment.predict(bag_of_words[:len(train)])
    y_label=np.array(train['sentiment']).reshape(-1,1)
    #y_pred=mnb_sentiment.predict(np.array(train['tfidf'].to_list()))
    '''
    ...
    #y_pred=mnb_sentiment.predict(np.array(test['tfidf'].to_list()))
