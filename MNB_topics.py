...
if __name__=='__main__':
    ...
    mnb_topic= MultinomialNB(fit_prior=True) 
    train=data_all[:len(train)]
    test=data_all[len(train):]

    mnb_topic.fit(bag_of_words[:len(train)],np.array(train['topic']))
    
    #mnb_topic.fit(np.array(train['tfidf'].to_list()),np.array(train['topic']).reshape(-1,1))
    y_pred=mnb_topic.predict(bag_of_words[:len(train)])
    y_label=np.array(train['topic']).reshape(-1,1)
    #y_pred=mnb_topic.predict(np.array(train['tfidf'].to_list()))
    '''
    ...
    #y_pred=mnb_topic.predict(np.array(test['tfidf'].to_list()))
