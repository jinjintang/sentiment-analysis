...
if __name__=='__main__':
    ...
    x_train=bag_of_words[:len(train)]
    y_train=train['topic']

    y_pred=np.array([[0]*20 for i in range(len(test))]).astype(np.float)
    score_sum = 0
    for train_idx, vali_idx in skf.split(x_train,y_train ):
        ...
        mnb_topic.fit(f_train_x, f_train_y)

        """对测试集进行预测"""
        y_test = mnb_topic.predict_proba(bag_of_words[len(train):])
        y_pred+=y_test
