...
if __name__=='__main__':
   ...
   # train=train[train['topic']!='neutral']
    ...
    dt_topic=DecisionTreeClassifier(criterion='entropy',min_samples_split=20,random_state=0)
    
    train=data_all[:len(train)]
    test=data_all[len(train):]

    dt_topic.fit(bag_of_words[:len(train)],np.array(train['topic']))

    #dt_topic.fit(np.array(train['tfidf'].to_list()),np.array(train['topic']).reshape(-1,1))
    y_pred=dt_topic.predict(bag_of_words[:len(train)])
    y_label=np.array(train['topic']).reshape(-1,1)
    #y_pred=dt_topic.predict(np.array(train['tfidf'].to_list()))
    '''
    ...
    #y_pred=dt_topic.predict(np.array(test['tfidf'].to_list()))
